...
uint8_t cyhal_spi_read_byte(cyhal_spi_t *spi, uint8_t send) {
  uint8_t received;
  (void)cyhal_spi_transfer(spi, &send, 1, &received, 1, 0);
  return received;
}
 ...
 */
void MFRC522_Init(mfrc522_t *mfrc, cyhal_gpio_t mosi, cyhal_gpio_t miso,
  ...
  /* Release RESET pin */
  cyhal_gpio_write(mfrc->m_reset, 1);
} // End constructor
 ...
 * The interface is described in the datasheet section 8.1.2.
 */
void PCD_ReadRegisterBytes(mfrc522_t *mfrc, uint8_t reg, uint8_t count,
  ...
  (void)cyhal_spi_send(&mfrc->m_SPI,
                       address); // Tell MFRC522 which address we want to read
  ...
  while (index < count) {
        ...
        rxAlign) // Only update bit positions rxAlign..7 in values[0]
    {
      ...
      uint8_t value = cyhal_spi_read_byte(&mfrc->m_SPI, address);
      ...
      // Read value and tell that we want to read the same address again.
      values[index] = cyhal_spi_read_byte(&mfrc->m_SPI, address);
    }
 ...
 */
void PCD_SetRegisterBits(mfrc522_t *mfrc, uint8_t reg, uint8_t mask) {
  uint8_t tmp = PCD_ReadRegister(mfrc, reg);
 ...
 */
void PCD_ClrRegisterBits(mfrc522_t *mfrc, uint8_t reg, uint8_t mask) {
  uint8_t tmp = PCD_ReadRegister(mfrc, reg);
 ...
 */
uint8_t PCD_CalculateCRC(mfrc522_t *mfrc, uint8_t *data, uint8_t length,
                         uint8_t *result) {
  ...
  PCD_SetRegisterBits(mfrc, FIFOLevelReg,
                      0x80); // FlushBuffer = 1, FIFO initialization
  PCD_WriteRegisterBytes(mfrc, FIFODataReg, length,
  ...
  while (1) {
    n = PCD_ReadRegister(
        mfrc, DivIrqReg); // DivIrqReg[7..0] bits are: Set2 reserved reserved
 ...
 */
void PCD_Init(mfrc522_t *mfrc) {
  /* Reset MFRC522 */
  cyhal_gpio_write(mfrc->m_reset, 0);
  ...
  PCD_WriteRegister(mfrc, TxASKReg,
  ...
  // TODO: Gain behaves quite weird...
  // PCD_WriteRegister(RFCfgReg, (0x07<<4)); // Set Rx Gain to max

  PCD_AntennaOn(mfrc); // Enable the antenna driver pins TX1 and TX2 (they were
 ...
 */
uint8_t PCD_TransceiveData(mfrc522_t *mfrc, uint8_t *sendData, uint8_t sendLen,
 ...
 */
uint8_t PCD_CommunicateWithPICC(mfrc522_t *mfrc, uint8_t command,
  ...
  PCD_SetRegisterBits(mfrc, FIFOLevelReg,
                      0x80); // FlushBuffer = 1, FIFO initialization
  PCD_WriteRegisterBytes(mfrc, FIFODataReg, sendLen,
  ...
  if (command == PCD_Transceive) {
    PCD_SetRegisterBits(mfrc, BitFramingReg,
  ...
  while (1) {
    n = PCD_ReadRegister(
        mfrc, ComIrqReg); // ComIrqReg[7..0] bits are: Set1 TxIRq RxIRq IdleIRq
  ...
  uint8_t errorRegValue = PCD_ReadRegister(
      mfrc, ErrorReg);        // ErrorReg[7..0] bits are: WrErr TempErr reserved
  ...
  if (backData && backLen) {
    n = PCD_ReadRegister(mfrc, FIFOLevelReg); // Number of bytes in the FIFO
    if (n > *backLen) {
      return STATUS_NO_ROOM;
    }

    *backLen = n; // Number of bytes returned
    PCD_ReadRegisterBytes(mfrc, FIFODataReg, n, backData,
                          rxAlign); // Get received data from FIFO
    _validBits =
        PCD_ReadRegister(mfrc, ControlReg) &
  ...
  if (backData && backLen && checkCRC) {
    ...
    n = PCD_CalculateCRC(mfrc, &backData[0], *backLen - 2, &controlBuffer[0]);
    if (n != STATUS_OK) {
      return n;
    }
 ...
 */
uint8_t PICC_RequestA(mfrc522_t *mfrc, uint8_t *bufferATQA,
                      uint8_t *bufferSize) {
 ...
 */
uint8_t PICC_WakeupA(mfrc522_t *mfrc, uint8_t *bufferATQA,
                     uint8_t *bufferSize) {
 ...
 * STATUS_TIMEOUT - probably due do bad antenna design.
 */
uint8_t PICC_REQA_or_WUPA(mfrc522_t *mfrc, uint8_t command, uint8_t *bufferATQA,
                          uint8_t *bufferSize) {
  uint8_t validBits;
  uint8_t status;
  ...
  PCD_ClrRegisterBits(mfrc, CollReg, 0x80);
 ...
 */
uint8_t PICC_Select(mfrc522_t *mfrc, Uid *uid, uint8_t validBits) {
  bool uidComplete;
  bool selectDone;
  bool useCascadeTag;
  ...
  while (!uidComplete) {
    ...
    while (!selectDone) {
      ...
      result = PCD_TransceiveData(mfrc, buffer, bufferUsed, responseBuffer,
                                  &responseLength, &txLastBits, rxAlign, false);
      if (result ==
          STATUS_COLLISION) { // More than one PICC in the field => collision.
        result = PCD_ReadRegister(
            mfrc, CollReg);  // CollReg[7..0] bits are: ValuesAfterColl reserved
 ...
 */
uint8_t PICC_HaltA(mfrc522_t *mfrc) {
  uint8_t result;
  uint8_t buffer[4];
 ...
 */
uint8_t PCD_Authenticate(mfrc522_t *mfrc, uint8_t command, uint8_t blockAddr,
  ...
  return PCD_CommunicateWithPICC(mfrc, PCD_MFAuthent, waitIRq, &sendData[0],
                                 sizeof(sendData), NULL, 0, NULL, 0, false);
 ...
 */
uint8_t MIFARE_Read(mfrc522_t *mfrc, uint8_t blockAddr, uint8_t *buffer,
                    uint8_t *bufferSize) {
 ...
 */
uint8_t MIFARE_Write(mfrc522_t *mfrc, uint8_t blockAddr, uint8_t *buffer,
                     uint8_t bufferSize) {
  uint8_t result;
  ...
  result = PCD_MIFARE_Transceive(mfrc, cmdBuffer, 2, false);
  if (result != STATUS_OK) {
    return result;
  }
  ...
  // Adds CRC_A and checks that the response is MF_ACK.
  result = PCD_MIFARE_Transceive(mfrc, buffer, bufferSize, false);
  if (result != STATUS_OK) {
    return result;
  }
 ...
 */
uint8_t MIFARE_UltralightWrite(mfrc522_t *mfrc, uint8_t page, uint8_t *buffer,
                               uint8_t bufferSize) {
  uint8_t result;
 ...
 */
static uint8_t MIFARE_TwoStepHelper(mfrc522_t *mfrc, uint8_t command,
                                    uint8_t blockAddr, uint32_t data) {
  uint8_t result;
  ...
  result = PCD_MIFARE_Transceive(mfrc, (uint8_t *)&data, 4, true);
  if (result != STATUS_OK) {
    return result;
  }
 ...
 */
uint8_t MIFARE_Decrement(mfrc522_t *mfrc, uint8_t blockAddr, uint32_t delta) {
 ...
 */
uint8_t MIFARE_Increment(mfrc522_t *mfrc, uint8_t blockAddr, uint32_t delta) {
 ...
 * memory.
 */
uint8_t MIFARE_Restore(mfrc522_t *mfrc, uint8_t blockAddr) {
 ...
 */
uint8_t MIFARE_Transfer(mfrc522_t *mfrc, uint8_t blockAddr) {
  uint8_t cmdBuffer[2]; // We only need room for 2 bytes.
  ...
  // Adds CRC_A and checks that the response is MF_ACK.
  return PCD_MIFARE_Transceive(mfrc, cmdBuffer, 2, false);
} // End MIFARE_Transfer()
 ...
 * MF_ACK or a timeout.
 */
uint8_t PCD_MIFARE_Transceive(mfrc522_t *mfrc, uint8_t *sendData,
  ...
  uint8_t validBits = 0;
  result =
      PCD_CommunicateWithPICC(mfrc, PCD_Transceive, waitIRq, cmdBuffer, sendLen,
 ...
 */
uint8_t PICC_GetType(mfrc522_t *mfrc, uint8_t sak) {
 ...
 */
char *PICC_GetTypeName(mfrc522_t *mfrc, uint8_t piccType) {
 ...
 */
char *GetStatusCodeName(mfrc522_t *mfrc, uint8_t code) {
 ...
 */
void MIFARE_SetAccessBits(mfrc522_t *mfrc, uint8_t *accessBitBuffer, uint8_t g0,
...
bool PICC_IsNewCardPresent(mfrc522_t *mfrc) {
  ...
  uint8_t result = PICC_RequestA(mfrc, bufferATQA, &bufferSize);
...
bool PICC_ReadCardSerial(mfrc522_t *mfrc) {
  uint8_t result = PICC_Select(mfrc, &mfrc->uid, 0);
