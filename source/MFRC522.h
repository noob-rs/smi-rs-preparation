 ...
 * Miguel Balboa (circuitito.com), Jan, 2012. Rewritten by Soren Thing Andersen
 * (access.thing.dk), fall of 2013 (Translation to English, refactored,
 * comments, anti collision, cascade levels.) Ported to mbed by Martin Olejar,
 * Dec, 2013
 *
 ...
 * Reader IC 3) The PICC (short for Proximity Integrated Circuit Card): A card
 * or tag using the ISO 14443A interface, eg Mifare or NTAG203.
 *
 ...
 * S20): Has 5 sectors * 4 blocks/sector * 16 bytes/block = 320 bytes. The
 * blocks are numbered 0-19. The last block in each sector is the Sector Trailer
 * like above.
 *
 * MIFARE Ultralight (MF0ICU1):
 *    Has 16 pages of 4 bytes = 64 bytes.
 *    Pages 0 + 1 is used for the 7-byte UID.
 *    Page 2 contains the last chech digit for the UID, one byte manufacturer
 ...
 * Ultralight C (MF0ICU2): Has 48 pages of 4 bytes = 64 bytes. Pages 0 + 1 is
 * used for the 7-byte UID. Page 2 contains the last chech digit for the UID,
...
/**
 * Setup GPIOs and SPI for the MFRC522.
 */
...
/**
 * Performs a soft reset on the MFRC522 chip and waits for it to be ready
 * again.
 */
...
/**
 * Transmits a Wake-UP command, Type A. Invites PICCs in state IDLE and HALT
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
uint8_t PICC_WakeupA(mfrc522_t *mfrc, uint8_t *bufferATQA, uint8_t *bufferSize);
 ...
 */
char *PICC_GetTypeName(mfrc522_t *mfrc, uint8_t type);
